...
func _ready() -> void:
	...
	product_manager.paid.connect(_on_paid)
	sprite.modulate.a = 0
	
func _on_product_returned(_amount: int, type: CircleManager.CircleTypes, price: int) -> void:
...
func _on_product_purchased(amount: int, type: CircleManager.CircleTypes, price: int) -> void:
	if is_hidden:
		show_crate()
...
func show_crate() -> void:
		...
		sprite.modulate.a = 0
		particle_container.modulate.a = 1
		tw.set_parallel()
...
func hide_crate() -> void:
		...
		sprite.position.y = 0
		tw.set_parallel()
		tw.tween_property(sprite, "position:y", -8, 0.1).set_ease(Tween.EASE_IN)
		tw.tween_property(sprite, "scale", Vector2(0.85, 1.15), 0.5).set_ease(Tween.EASE_IN_OUT)
		tw.tween_property(sprite, "position:y", 20, 0.15).set_ease(Tween.EASE_OUT).set_delay(0.2)
		tw.tween_property(sprite, "modulate:a", 0, 0.1).set_ease(Tween.EASE_IN_OUT).set_delay(0.27)
		tw.tween_property(particle_container, "modulate:a", 0, 0.1).set_ease(Tween.EASE_IN_OUT).set_delay(0.27)
